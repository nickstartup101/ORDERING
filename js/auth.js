...
// 1. Reset Session Timer ເມື່ອມີການເຄື່ອນໄຫວ
...
// ດັກຈັບການເຄື່ອນໄຫວຂອງຜູ້ໃຊ້ທັງຫມົດ
...
function autoLogoutSession() {
  ...
  showAtelierAlert({
    title: "ໝົດເວລາເຊດຊັນ (Session Timeout)",
    message: "ທ່ານບໍ່ມີການເຄື່ອນໄຫວເກີນ 30 ນາທີ ລະບົບໄດ້ອອກຈາກລະບົບອັດຕະໂນມັດເພື່ອຄວາມປອດໄພ",
    type: "info"
  });
}
...
function toggleAuthMode() {
  ...
  if (isRegisterMode) {
    ...
    btn.textContent = "ລົງທະບຽນ (Sign Up)";
    toggle.textContent = "ມີບັນຊີແລ້ວ? ເຂົ້າສູ່ລະບົບ";
  } else {
    nameBox.classList.add('hidden');
    title.textContent = "ເຂົ້າສູ່ລະບົບ";
    btn.textContent = "ເຂົ້າສູ່ລະບົບ (Sign In)";
    toggle.textContent = "ຍັງບໍ່ມີບັນຊີ? ລົງທະບຽນໃໝ່ທີ່ນີ້";
  }
}
...
function handleAuthSubmit() {
  ...
  showAtelierAlert({
    title: "ຍິນດີຕ້ອນຮັບ!",
    message: `ສະບາຍດີ, ${currentUser.name} ເຂົ້າສູ່ລະບົບສຳເລັດແລ້ວ. ເຊດຊັນຈະມີອາຍຸ 30 ນາທີ.`,
    type: "success"
  });
}
...
function logoutUser() {
  ...
  updateUserSessionUI();
  dispatchRoleView('customer');
  showAtelierAlert({ title: "ສຳເລັດ", message: "ທ່ານໄດ້ອອກຈາກລະບົບຮຽບຮ້ອຍແລ້ວ", type: "info" });
}
...
// 2. ຈັດການຂໍ້ມູນ Profile ແບບຄົບວົງຈອນ (ຮູບ, ວັນເກີດ, ເບີໂທ, ປ່ຽນລະຫັດ)
function renderCustomerProfile() {
  ...
  const avatarDisplay = currentUser.profileImage 
    ...
    : `<div class="w-20 h-20 rounded-full bg-forest-gradient text-white flex items-center justify-center font-serif text-[28px] shadow-xs">${currentUser.name.charAt(0).toUpperCase()}</div>`;
    ...
    <div class="max-w-lg mx-auto space-y-5">
      ...
      <div class="p-6 rounded-2xl bg-surface-pure border border-hairline space-y-5 shadow-xs">
        ...
        <div class="space-y-3 pt-2 border-t border-hairline text-[12px]">
          ...
          <div>
            <label class="block text-taupe mb-1">ວັນເດືອນປີເກີດ (Date of Birth) ສຳລັບຂອງຂວັນວັນເກີດ:</label>
      ...
      <div class="p-6 rounded-2xl bg-surface-pure border border-hairline space-y-3 shadow-xs">
        ...
        <div id="customerHistoryList" class="divide-y divide-hairline border border-hairline rounded-xl bg-surface">
...
async function handleProfilePhotoUpload(e) {
  ...
  try {
    ...
    updateUserSessionUI();
    showAtelierAlert({ title: "ສຳເລັດ", message: "ອັບເດດຮູບໂປຣໄຟລ໌ຮຽບຮ້ອຍແລ້ວ!", type: "success" });
  } catch (err) {
    showAtelierAlert({ title: "ຜິດພາດ", message: "ບໍ່ສາມາດອັບໂຫຼດຮູບໄດ້", type: "error" });
  }
}

function verifyPhoneAction() {
  if (currentUser.isPhoneVerified) {
    showAtelierAlert({ title: "ແຈ້ງເຕືອນ", message: "ເບີໂທລະສັບນີ້ໄດ້ຮັບການຢືນຢັນແລ້ວ", type: "info" });
    return;
  }
  ...
  if (otp === "8899") {
    ...
    saveCurrentUserData();
    renderCustomerProfile();
    showAtelierAlert({ title: "ສຳເລັດ", message: "ເບີໂທລະສັບຂອງທ່ານໄດ້ຮັບການຢືນຢັນ OTP ຮຽບຮ້ອຍແລ້ວ!", type: "success" });
  } else if (otp) {
    showAtelierAlert({ title: "ຜິດພາດ", message: "ລະຫັດ OTP ບໍ່ຖືກຕ້ອງ!", type: "error" });
  }
}
...
function saveProfileDetails() {
  ...
  if (!name) {
    showAtelierAlert({ title: "ແຈ້ງເຕືອນ", message: "ກະລຸນາໃສ່ຊື່ຂອງທ່ານ!", type: "info" });
    return;
  }
  ...
  renderCustomerProfile();
  updateUserSessionUI();
  showAtelierAlert({ title: "ສຳເລັດ", message: "ບັນທຶກຂໍ້ມູນໂປຣໄຟລ໌ຮຽບຮ້ອຍແລ້ວ!", type: "success" });
}
...
function changeCustomerPassword() {
  ...
  if (!newPass || newPass.length < 3) {
    showAtelierAlert({ title: "ແຈ້ງເຕືອນ", message: "ລະຫັດຜ່ານໃໝ່ຕ້ອງມີຢ່າງໜ້ອຍ 3 ຕົວອັກສອນ!", type: "info" });
    return;
  }

  if (newPass !== confirmPass) {
    showAtelierAlert({ title: "ຜິດພາດ", message: "ລະຫັດຜ່ານທັງສອງຊ່ອງບໍ່ກົງກັນ!", type: "error" });
    return;
  }
  ...
  showAtelierAlert({ title: "ສຳເລັດ", message: "ປ່ຽນລະຫັດຜ່ານໃໝ່ຮຽບຮ້ອຍແລ້ວ!", type: "success" });
}

function saveCurrentUserData() {
  localStorage.setItem('ladolce_user', JSON.stringify(currentUser));
  // Sync back to accounts list
  ...
  container.innerHTML = myOrders.slice(0, 5).map(o => `
    <div class="p-3.5 flex items-center justify-between text-[12px]">
      <div>
        ...
        <span class="text-[10px] text-taupe">${new Date(o.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
      </div>
